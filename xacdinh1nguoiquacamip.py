...
video_url = "http://192.168.1.209:5000/video_feed"
...
def message_sender():
    while True:
        ...
        print(f"Sending message for posture: {posture}")  # Debugging line
...
async def process_frame():
    ...
    while True:
        ...
        if a != -1 and b != -1:
            ...
            posture = None  # Khởi tạo posture mặc định
            ...
            for i, result in enumerate(results):  
                if result.keypoints is not None:
                    ...
                    if keypoints[0].shape[0] > 11:  # Đảm bảo có đủ keypoints để truy cập
                        ...
                        if posture != previous_posture:
                            ...
                            # Nếu khoảng cách giữa các thay đổi tư thế lớn hơn hoặc bằng 0.5 giây, quay video
                                ...
                                video_thread = threading.Thread(target=lambda: message_queue.put((posture, record_video(frame), True)))
                                video_thread.start()
                    ...
                    # Hiển thị tư thế
                    if posture:  # Chỉ hiển thị nếu posture đã được xác định
